...
export default function MyLoans() {
  ...
  useEffect(() => {
    ayudaApi.list().then(setLoans).catch((requestError) => setError(requestError.message)).finally(() => setLoading(false))
  }, [])
  ...
  const paymentRows = loans.flatMap(buildPaymentRows).sort((a, b) => new Date(b.paidAt) - new Date(a.paidAt))
  ...
  return <div className="space-y-6">
    <section className="relative overflow-hidden rounded-[22px] border border-[#313335] bg-[linear-gradient(120deg,#191b1d_0%,#25282b_62%,#34312b_100%)] p-5 text-white shadow-[0_16px_38px_rgba(27,29,31,0.15)] sm:p-6">
      <div className="relative flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl border border-blue-300/50 bg-white/10 text-blue-200"><HandCoins size={20} /></div><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-200">My Account</p><h1 className="mt-1 text-2xl font-black sm:text-[30px]">My Loans</h1><p className="mt-1 text-xs text-blue-100/80 sm:text-sm">Read-only loan balance and commission deduction records.</p></div></div>
    </section>
