...
function TeamSubteamDetails({ details, loading, onClose }) {
  ...
  return <div className="rounded-[30px] bg-white p-5 shadow-sm sm:p-8">
    ...
    <div className="overflow-hidden rounded-3xl border border-[#313335] bg-gradient-to-br from-[#191b1d] via-[#25282b] to-[#40382c] p-6 text-white shadow-lg shadow-black/10 sm:p-8"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><div className="mb-3 inline-flex rounded-full bg-white/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.2em] text-[#e0b45f]">Team Overview</div><h2 className="text-4xl font-black tracking-tight sm:text-5xl">{details.teamName}</h2><div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm text-white/65"><p>EVP <strong className="ml-1 text-white">{details.evpName || 'Unassigned'}</strong></p><span className="hidden text-white/30 sm:inline">•</span><p>Established <strong className="ml-1 text-white">{details.establishedDate ? new Date(`${details.establishedDate}T00:00:00`).toLocaleDateString() : 'Not set'}</strong></p></div></div><div className="grid grid-cols-3 gap-2 sm:gap-3"><SummaryStat value={details.subteams?.length || 0} label="Sub-teams" /><SummaryStat value={assignedMembers.length} label="Members" /><SummaryStat value={weeklyRecruits} label="New this week" accent /></div></div></div>
